...
public interface EnrollmentRepository extends JpaRepository<Enrollment, Integer> {
    ...
    @Query("SELECT l " + "FROM Enrollment l " + "JOIN l.course c "
            + "WHERE c.code = :courseCode AND c.isDelete = false AND l.isDelete = false AND l.classArise.code = :classCode")
    ...
    @Modifying
    @Query("UPDATE Enrollment e SET e.isDelete = true WHERE e.student.id IN (:id) AND e.classArise.id = :classId")
    void softDeleteEnrollmentMappingByListStudent(@Param("id") List<Integer> id,
            @Param("classId") int classId);

    @Modifying
    @Query("UPDATE Enrollment e SET e.isDelete = true WHERE e.classArise.id = :id")

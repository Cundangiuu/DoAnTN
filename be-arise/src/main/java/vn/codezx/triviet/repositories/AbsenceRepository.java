...
public interface AbsenceRepository extends JpaRepository<Absence, Integer> {
  ...
  @Query("SELECT a FROM Absence a WHERE a.classTvms.code = :classCode AND a.isDelete = false")
  ...
  @Modifying
  @Query("UPDATE Absence a SET a.isDelete = true WHERE a.student.id IN (:id) AND a.classTvms.id = :classId")
  void softDeleteAbsenceMappingByListStudent(@Param("id") List<Integer> id,
      @Param("classId") int classId);


  @Modifying
  @Query("UPDATE Absence a SET a.isDelete = true WHERE a.classTvms.id = :id")

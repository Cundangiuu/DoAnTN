...
public interface StudentRepository extends JpaRepository<Student, Integer> {
    ...
    @Query("SELECT s FROM Student s JOIN Enrollment e ON s.id = e.student.id WHERE e.classTvms.code = :classCode AND e.isDelete = false AND s.isDelete = false")
    ...
    @Query(
            value = "SELECT s FROM Student s WHERE s.isDelete = false AND s.id NOT IN (SELECT e.student.id FROM Enrollment e WHERE e.classTvms IS NULL AND e.isDelete = false) AND s.id NOT IN (SELECT i.enrollment.student.id FROM Invoice i WHERE i.invoiceStatus = 'NOT_PAID' AND i.isDelete = false)")
    ...
    @Query(
            value = "SELECT s FROM Student s WHERE s.id IN :ids AND s.id NOT IN (SELECT st.id FROM ClassTvms c JOIN c.students st WHERE c.id = :classId)")

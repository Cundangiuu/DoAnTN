...
public class ClassDayServiceImpl implements ClassDayService {
  ...
  public List<ClassDayDTO> generateClassDay(String requestId, ClassTvms classTvms,
    ...
    var classDays = generateDates(classTvms.getStartDate(), classTvms.getSchedules(),
        classTvms.getCourse().getLessons().size(), holidays);

    var days = classTvms.getClassDays();
    var lessons = classTvms.getCourse().getLessons();
    ...
    for (int i = 0; i < lessons.size(); i++) {
      ...
      classDay.setClassTvms(classTvms);

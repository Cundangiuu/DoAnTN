...
import vn.codezx.arise.services.impl.CourseServiceImpl;
...
class CourseServiceImplTest {
  ...
  @Mock
  private ClassAriseRepository classAriseRepository;
  ...
  void testGetCourseByCode_NotFound() {
    ...
    AriseException exception = assertThrows(AriseException.class, () ->
  ...
  void testDeleteCourse() {
    when(courseRepository.findByCodeAndIsDeleteFalse("COU12012401")).thenReturn(courseEntity);
    when(classAriseRepository.existsByCourseId(courseEntity.getId())).thenReturn(false);
    doAnswer(invocation -> {
      Course course = invocation.getArgument(0);
      return CourseDTO.builder().id(course.getId()).code(course.getCode()).name(course.getName())
          .tuitionRate(course.getTuitionRate()).numberHour(course.getNumberHour())
  ...
  void testDeleteCourseByCode_CourseNotFound() {
    ...
    AriseException exception = assertThrows(
        AriseException.class,
  ...
  void testDeleteCourseByCode_HasAssociatedClasses() {
    ...
    when(classAriseRepository.existsByCourseId(courseEntity.getId())).thenReturn(true);
